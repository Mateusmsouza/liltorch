...
class Convolutional(Layer):
    ...
    def __init__(self, kernel_size: tuple[int, int], n_filers: int, padding: str, stride: int =1 ):
        super().__init__()
    ...
    def _config_padding(self, height, filter_shape_h, width, filter_shape_w):
        ...
        if self.padding == 'same':
            ...
            Exception(f'padding {self.padding} is an invalid option. Use valid or same.')
    ...
    def _apply_padding(self, input, n_col_padding):
        ...
        '''
        return np.pad(input, ((0, 0), (n_col_padding[0], n_col_padding[0]), (n_col_padding[1], n_col_padding[1]), (0, 0)), 'constant')
    ...
    def forward(self, x_input: np.ndarray):
        ...
        X_pad = self._apply_padding(input=x_input, n_col_padding=(pad_height, pad_width))
